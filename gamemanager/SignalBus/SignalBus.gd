...
signal trigger_routine_by_id(routine_id: String, force_world_position: bool, input_position: Vector2, input_rotation: float, parent_node: Variant)

# ==================== 卡牌 (Card) 相关信号 ====================
## 尝试添加卡牌时发出。
## @param id: Variant - 卡牌的唯一标识符。
signal try_add_card(id: String)
## 确认添加卡牌后发出。
## @param card_info: Dictionary/Object - 包含卡牌的详细信息。
signal add_card(card_info: Dictionary)
## 尝试删除/移除卡牌时发出。
## @param id: Variant - 要删除的卡牌的唯一标识符。
signal del_card(id: String)
## 尝试禁用/Ban 卡牌时发出。
## @param id: Variant - 要禁用的卡牌的唯一标识符。
signal ban_card(id: String)
## 玩家尝试使用卡牌时发出 (可能在使用前需要检查条件或消耗资源)。
## @param id: Variant - 要使用的卡牌的唯一标识符。
## @param cost_rate: float (可选) - 使用卡牌的成本倍率 (例如，某些效果可能增加消耗)。
signal use_card(id: String, cost_rate: float)
## 确认卡牌成功使用后发出 (条件满足，资源已消耗)。
## @param id: Variant - 成功使用的卡牌的唯一标识符。
signal true_use_card(id: String)
## 请求切换卡牌选择时发出 (例如按左右键切换)。
## @param bias: int - 切换方向和幅度 (例如 +1 向右切换一个，-1 向左切换一个)。
signal card_select_next(bias: int)
...
signal try_add_passive(id: String)
## 确认添加被动技能后发出。
## @param psv_info: Dictionary/Object - 包含被动技能的详细信息。
signal add_passive(psv_info: Dictionary)
## 尝试删除/移除被动技能时发出。
## @param id: Variant - 要删除的被动技能的唯一标识符。
signal del_passive(id: String)
## 尝试禁用/Ban 被动技能时发出。
## @param id: Variant - 要禁用的被动技能的唯一标识符。
signal ban_passive(id: String)
...
signal player_invincible(seconds: float)
## 给玩家添加状态效果 (Buff) 时发出。
...
signal player_add_buff(buff_id: String, buff_intensity: float, buff_source: String)
## 根据来源移除玩家身上的 Buff 时发出。
## @param source: Variant - 要移除的 Buff 的来源标识。
signal player_del_buff_by_source(source: String)
...
signal upgrade_group(group: String, currentlevel: Node)

signal renew_state(id: String)
## 当某个东西 (技能、属性等) 达到最高等级时发出。
## @param anyname: Variant - 达到最高级的项目名称或标识符。
signal upgrade_max(anyname: String)
## 激活某个 CP (组件/特性点/?) 时发出。
## @param cp_info: Dictionary/Object - 激活的 CP 的信息。
signal cp_active(cp_info: Dictionary)
## 删除/移除某个 CP 时发出。
## @param cp_id: Variant - 要删除的 CP 的标识符。
signal cp_del(cp_id: String)
signal delete_mode(is_on: bool, completed: bool)
...
signal drop(id: String, global_position: Vector2, value: Variant)
...
signal fly_to_player(exp_buff: float, mana_buff: float, score_buff: float, point_ratio_buff: Variant)

# ==================== 飞碟事件 (UFO) ====================
## 拾起飞碟钥匙后请求开始一场飞碟事件。
## @param color: int - 1=红 2=绿 3=蓝
## @param pickup_position: Vector2 - 拾起位置
signal ufo_spawn_requested(color: int, pickup_position: Vector2)
## 飞碟被玩家击破，携带账本快照进入结算。
## @param ufo: Node - 被击破的飞碟实例
## @param ledger: Dictionary - exp/mana/score/fragment_count
## @param color: int - 1=红 2=绿 3=蓝
## @param kill_global_position: Vector2 - 击破位置
signal ufo_killed(ufo: Node, ledger: Dictionary, color: int, kill_global_position: Vector2)

signal d4c_create(id: String, position: Vector2, parent: Node, damage: float, callback: Variant)
# ==================== 调试/控制 相关信号 ====================
signal clear_enemy(not_drop: bool)
signal pause_spawner(is_pause: bool)
signal add_mob_to_manager(mob_node: Node)
...
signal shutter
signal focus_on(type: String, id: String)
signal focus_off
# ==================== 调试日志 (Debug Log) ====================
...
func _ready() -> void:
	if is_log:
		var signal_dic: Array[Dictionary] = get_signal_list() # 获取此脚本定义的所有信号
		...
		for sig in signal_dic:
			...
			var callable: Callable = Callable(self, "_on_signal_emit")
			var bound_callable: Callable = callable.bind('[color=green][b]SIGNAL EMITTED:[/b][/color] ' + str(signal_name))
			var err: Error = connect(signal_name, bound_callable)
			if err != OK:
...
func _on_signal_emit(
		message1: Variant = "", # 可能捕获信号的第1个参数 (如果存在且类型匹配)
		message2: Variant = "", # 接收由 bind() 传入的信号名称字符串
		message3: Variant = "", # 可能捕获信号的第2个参数
		message4: Variant = "", # 可能捕获信号的第3个参数
		message5: Variant = "", # 可能捕获信号的第4个参数
		message6: Variant = ""  # 可能捕获信号的第5个参数
	) -> void:
