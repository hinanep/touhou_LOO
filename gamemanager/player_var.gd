...
var change_times: int = 0: #- 刷新排除次数：增加刷新与排除的次数。前者可刷新升级时可选的记忆结晶，后者可使选择的记忆结晶在本局游戏剩余时间内不再出现
	set(value):
		if change_times == value:
			return
		var delta := value - change_times
		change_times = value
		if delta > 0:
			change_times_remaining += delta
var change_times_remaining: int = 0 #- 本局剩余刷新/遗忘共享次数
...
func new_scene() -> void:
	...
	_bonus_pick_queue = 0
	change_times_remaining = change_times
	RunSession.begin_run()


## 消耗一次刷新/遗忘共享次数；成功返回 true
func try_consume_change_time() -> bool:
	if change_times_remaining <= 0:
		return false
	change_times_remaining -= 1
	return true
