...
func _init(p_prefab: PackedScene, p_parent_node: Node) -> void:
	self.prefab = p_prefab
	self.parent_node = p_parent_node

# 从池中获取一个对象
func get_object(parent: Node = parent_node) -> Node:
	...
	# 如果池是空的，创建一个新的
	if prefab:
		var new_obj: Node = prefab.instantiate()
		new_obj.process_mode = Node.PROCESS_MODE_PAUSABLE
...
func disable_object(obj: Node) -> void:
