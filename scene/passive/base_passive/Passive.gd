...
func upgrade_passive(group: String, cur: int) -> void:
	if group != psv_info.upgrade_group:
		return
	level+=1
	var intensity: float = table.Upgrade[psv_info.upgrade_group].buff_addition[level-1]
	trigger_buff(intensity)

	if(level == max_level):
		SignalBus.upgrade_max.emit(psv_info.id)
func del(id: String) -> void:
	if id == psv_info.id:
		SignalBus.player_del_buff_by_source.emit(psv_info.id)
		queue_free()

func trigger_buff(intensity: float) -> void:
