...
# 存储所有创建的pp预制体实例
var pp_list: Array[RichTextLabel] = []

# 攻击类属性列表
var attack_properties: Array[String] = [
...
# 防御类属性列表
var defense_properties: Array[String] = [
	"player_hp_max",
	"hp_max_ex_percent",
	"player_hp_regen",
	"lifesteal",
...
# 成长类属性列表
var growth_properties: Array[String] = [
	"range_pick",
	"luck",
	"experience_ratio",
	"point_ratio",
...
# 存储预览的属性变化
var preview_changes: Dictionary = {}
...
func gen_properties() -> void:
	...
	# 获取pp预制体
	var pp_scene: PackedScene = PresetManager.getpre('property_text')
...
func create_property_section(section_name: String, properties: Array, pp_scene: PackedScene) -> void:
	# 创建分类标题
	var title_pp: RichTextLabel = pp_scene.instantiate()
	add_child(title_pp)
	...
	for property in properties:
		var pp: RichTextLabel = pp_scene.instantiate()
		add_child(pp)
		pp_list.append(pp)
		update_property_text(pp, property)

func update_property_text(pp: RichTextLabel, property_name: String) -> void:
	var property_value: Variant = player_var.get(property_name)
	var display_name: String = property_names.get(property_name, property_name)

	# 格式化数值显示
	var value_text: String = _StatDisplayFormat.format_property(property_name, property_value)

	# 检查是否有预览变化
	if preview_changes.has(property_name):
		var preview_value: Variant = preview_changes[property_name]
...
func set_pp_text(pp: RichTextLabel, text: String) -> void:
	pp.text = text

# 更新所有属性显示
func update_all_properties() -> void:
	for i in range(pp_list.size()):
		var pp: RichTextLabel = pp_list[i]
		# 跳过标题行
		if i == 0 or i == len(attack_properties) + 1 or i == len(attack_properties) + len(defense_properties) + 2:
			continue

		# 计算当前属性在哪个分类中
		var property_index: int = 0
		var current_properties: Array = []
...
# 设置属性变化预览
func set_property_preview(property_name: String, new_value: float) -> void:
	preview_changes[property_name] = new_value

# 清除属性变化预览
func clear_property_preview(property_name: String = "") -> void:
...
# 根据被动技能ID设置属性变化预览
func set_passive_preview(passive_id: String) -> void:
	clear_property_preview()
	if not table.Passive.has(passive_id):
		return
	var passive_info: Dictionary = table.Passive[passive_id]
	var upgroup: String = table.Passive[passive_id].upgrade_group
	var newlevel: int = RunSession.PassiveManager.get_passive_level(passive_id)+1
	...
	for property in table.Upgrade[upgroup]:
		...
		for buff_id in passive_info.buff:
			if not table.Buff.has(buff_id):
				continue
			var buff_info: Dictionary = table.Buff[buff_id]
